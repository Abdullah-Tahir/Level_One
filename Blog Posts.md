...
<h3>A private key:</h3> There is a key associated with each public key that is a thing called “private key” .A private key  puts signatures on all the transactions.Think of it this way, we need public key to actually sign into the blockchain, once we sign in people would be able to see it using your public. But they cannot sign stuff on your behalf.The thing to note is that only you can do the signature using the private key. YOU DO NOT share it with anyone, hence you can write/sign but everyone can verify. Obviously this is just the skim of it however what you really to need to know is the your Private key should be left PRIVATE and your public key PUBLIC.The private key is used to decrypt the data, the private key mechanism is faster then then public key mechanism
